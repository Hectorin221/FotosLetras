...
class detectormanos():
    ...
    def __init__(self, mode=False, maxManos=2, modelo_complexity=1, Confdeteccion=0.5, Confsegui=0.5):
        self.mode = mode
        self.maxManos = maxManos
        self.compl = modelo_complexity
        ...
        self.dibujo = mp.solutions.drawing_utils  # Agregado para poder dibujar landmarks
    ...
    def encontrarmanos(self, frame, dibujar=True):
        ...
        return frame  # Retorna el frame modificado
    ...
    def encontrarposicion(self, frame, ManoNum=0, dibujarPuntos=True, dibujarBox=True, color=(255,0,0)):
        ...
        if self.resultados.multi_hand_landmarks:
            ...
            for id, lm in enumerate(miMano.landmark):
                ...
                cx, cy = int(lm.x * ancho), int(lm.y * alto)  # Cambiado el orden para ancho y alto correcto
                xlista.append(cx)
                ylista.append(cy)
...
def main():
    ...
    while True:
        ...
        # ---------------- Mostrar FPS ----------------
        ctiempo = time.time()
        fps = 1 / (ctiempo - ptimepo) if (ctiempo - ptimepo) > 0 else 0
        ptimepo = ctiempo
